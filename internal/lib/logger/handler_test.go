...
func TestRequestId(t *testing.T) {
	var b bytes.Buffer
	logger := slog.New(NewHandler(io.Writer(&b)))
	logger.DebugContext(context.WithValue(context.Background(), ContextKeyTraceId, "01951302-6642-7007-9539-4c0cc944e4eb"), "Test")

	r := regexp.MustCompile(`{"time":".+?",(.+),"id":".+?"}\n`)
	got := r.ReplaceAllString(b.String(), "$1")

	assert.Equal(t, got, fmt.Sprintf(`"level":"DEBUG","msg":"Test","%s":"01951302-6642-7007-9539-4c0cc944e4eb"`, ContextKeyTraceId))
}
